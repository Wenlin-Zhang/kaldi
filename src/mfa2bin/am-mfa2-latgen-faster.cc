// mfa2bin/am-mfa2-latgen-faster.cc
...
bool ProcessUtterance(LatticeFasterDecoder &decoder,
  ...
  DecodableAmMfa2Scaled ammfa2_decodable(am_mfa2, trans_model, features, acoustic_scale);

  return DecodeUtteranceLatticeFaster(
      decoder, ammfa2_decodable, trans_model, word_syms, utt, acoustic_scale, determinize,
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage:  am-mfa2-latgen-faster [options] <model-in> (<fst-in>|<fsts-rspecifier>) "
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) { // a single FST.
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          double like;
          if (ProcessUtterance(decoder, am_mfa2, trans_model, acoustic_scale,
                               features,word_syms,
