...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Accumulate stats for Am-Mfa training.\n"
        "Usage: am-mfa-acc-stats [options] <model-in> <feature-rspecifier> <posteriors-rspecifier> <stats-out>\n"
        "e.g.: am-mfa-acc-stats 1.mdl 1.ali scp:train.scp 'ark:ali-to-post 1.ali ark:-|' 1.acc\n";
      ...
      if (!posteriors_reader.HasKey(utt)) {
        ...
        for (size_t i = 0; i < posterior.size(); i++) {
          ...
          for (size_t j = 0; j < posterior[i].size(); j++) {
            ...
            tot_like_this_file += ammfa_accs.Accumulate(am_mfa, mat.Row(i), &spk_vars, pdf_id,
                                                       weight, acc_flags, this_gselect)
                                                       * weight;
            tot_weight += weight;
          }
        }
        ammfa_accs.CommitSpkr(&spk_vars, acc_flags);
