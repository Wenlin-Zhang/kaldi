...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Sum multiple accumulated stats files for AmMfa training.\n"
        "Usage: am-mfa-sum-accs [options] stats-out stats-in1 stats-in2 ...\n";
    ...
    for (int i = 2, max = po.NumArgs(); i <= max; i++) {
      ...
      ammfa_accs.Read(ki.Stream(), binary_read, true /* add values */);
    }
