// mfabin/am-mfa-align-compiled.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Align features given [MFA-based] models.\n"
        "Usage: am-mfa-align-compiled [options] model-in graphs-rspecifier "
        "feature-rspecifier alignments-wspecifier\n"
        "e.g.: am-mfa-align-compiled 1.mdl ark:graphs.fsts scp:train.scp ark:1.ali\n";
    ...
    for (; !fst_reader.Done(); fst_reader.Next()) {
      ...
      if (!feature_reader.HasKey(utt)) num_no_feat++;
        ...
        DecodableAmMfaScaled am_mfa_decodable(am_mfa, spk_vars, trans_model, features, *gselect, acoustic_scale);

        decoder.Decode(&am_mfa_decodable);
