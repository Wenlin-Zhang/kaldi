// mfabin/am-mfa-rescore-lattice.cc
...
void LatticeAcousticRescore(const AmMfa &am,
      ...
      if (arc_id == -1) { // Final state
        ...
        if (trans_id != 0) {  // Non-epsilon input label on arc
          ...
          if (pdf_id_to_like.count(pdf_id) == 0) {
            ll = am.LogLikelihood(pdf_id, data.Row(t), &spk_vars, NULL);
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: am-mfa-rescore-lattice [options] <model-in> <lattice-rspecifier> "
        "<feature-rspecifier> <lattice-wspecifier>\n"
        " e.g.: am-mfa-rescore-lattice 1.mdl ark:1.lats scp:trn.scp ark:2.lats\n";
    ...
    for (; !compact_lattice_reader.Done(); compact_lattice_reader.Next()) {
      ...
      DecodableAmMfa am_mfa_decodable(am_mfa, spk_vars, trans_model, feats, *gselect);

      if (kaldi::RescoreCompactLattice(&am_mfa_decodable, &clat)) {
