// mfabin/am-mfa-acc-stats-gpost.cc
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Accumulate stats for AmMfa training, given Gaussian-level posteriors\n"
        "Usage: am-mfa-acc-stats-gpost [options] <model-in> <feature-rspecifier> "
        "<gpost-rspecifier> <stats-out>\n"
        "e.g.: am-mfa-acc-stats-gpost 1.mdl 1.ali scp:train.scp ark, s, cs:- 1.acc\n";
    ...
    po.Register("update-flags", &update_flags_str, "Which AmMfa parameters to update: subset of yMwStN.");
    ...
    ammfa_accs.ResizeAccumulators(am_mfa);
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (!gpost_reader.HasKey(utt)) {
        ...
        if (!(acc_flags & kAmMfaRemoveSpkrProjections)) {
        ...
        if (!(acc_flags & kAmMfaRemoveSpkrProjections))
          ammfa_accs.BeginSpkr(&spk_vars, acc_flags);
        for (size_t i = 0; i < gpost.size(); i++) {
          for (size_t j = 0; j < gpost[i].size(); j++) {
            ...
            if (!(acc_flags & kAmMfaRemoveSpkrProjections))
              ammfa_accs.AccumulateFromPosteriors(am_mfa, gpost[i][j].second, mat.Row(i),
                                                &spk_vars, pdf_id, acc_flags);
            else
              ammfa_accs.AccumulateFromPosteriors(am_mfa, gpost[i][j].second, mat.Row(i),
                                                       NULL, pdf_id, acc_flags);
            tot_weight += weight;
          }
        }
        if (!(acc_flags & kAmMfaRemoveSpkrProjections))
          ammfa_accs.CommitSpkr(&spk_vars, acc_flags); // no harm doing it per utterance.
...

    {
      ...
      ammfa_accs.Write(ko.Stream(), binary);
    }
