// mfabin/am-mfa-latgen-faster.cc
...
bool ProcessUtterance(LatticeFasterDecoder &decoder,
  ...
  DecodableAmMfaScaled ammfa_decodable(am_mfa, spk_vars, trans_model, features, *gselect, acoustic_scale);

  return DecodeUtteranceLatticeFaster(
      decoder, ammfa_decodable, trans_model, word_syms, utt, acoustic_scale, determinize,
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Decode features using MFA-based model.\n"
        "Usage:  am-mfa-latgen-faster [options] <model-in> (<fst-in>|<fsts-rspecifier>) "
        "<features-rspecifier> <lattices-wspecifier> [<words-wspecifier> [<alignments-wspecifier>] ]\n";
    ParseOptions po(usage);
    BaseFloat acoustic_scale = 0.1;
    bool allow_partial = false;
    string word_syms_filename, gselect_rspecifier, spkvecs_rspecifier, utt2spk_rspecifier;
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) { // a single FST.
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          double like;
          if (ProcessUtterance(decoder, am_mfa, trans_model, acoustic_scale,
