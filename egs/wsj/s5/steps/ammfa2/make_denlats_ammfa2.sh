...
# Create denominator lattices for MMI/MPE training, with AmMfa2 models.  If the
...
if [ $# != 4 ]; then
   echo "Usage: steps/make_denlats_ammfa2.sh [options] <data-dir> <lang-dir> <src-dir|alidir> <exp-dir>"
   echo "  e.g.: steps/make_denlats_ammfa2.sh data/train data/lang exp/ammfa4a_ali exp/ammfa4a_denlats"
...
if [ $sub_split -eq 1 ]; then 
  $cmd JOB=1:$nj $dir/log/decode_den.JOB.log \
   am-mfa2-latgen-faster  $spkvecs_opt --beam=$beam --lattice-beam=$lattice_beam --acoustic-scale=$acwt \
  ...
  for n in `seq $nj`; do
    if [ -f $dir/.done.$n ] && [ $dir/.done.$n -nt $alidir/final.mdl ]; then
      ...
      $cmd JOB=1:$sub_split $dir/log/$n/decode_den.JOB.log \
        am-mfa2-latgen-faster  $spkvecs_opt_subset --beam=$beam --lattice-beam=$lattice_beam \
...
echo "$0: done generating denominator lattices with AmMfa2."
