...
# This script does decoding with an AmMfa2 system, by rescoring lattices
# generated from a previous AmMfa2 system.  Basically it rescores
...
if [ $# -ne 4 ]; then
  echo "Usage: steps/decode_ammfa2_rescore.sh [options] <graph-dir|lang-dir> <data-dir> <old-decode-dir> <decode-dir>"
  echo " e.g.: steps/decode_ammfa2_rescore.sh --transform-dir exp/tri3b/decode_dev93_tgpr \\"
  echo "      exp/ammfa3a/graph_tgpr data/test_dev93 exp/ammfa3a/decode_dev93_tgpr exp/ammfa3a_mmi/decode_dev93_tgpr"
...
for f in $graphdir/words.txt $data/feats.scp $olddir/lat.1.gz $srcdir/$iter.mdl; do
...
if [ ! -z "$transform_dir" ]; then
  ...
  echo "$0: **WARNING**: you seem to be using an AmMfa system trained with transforms,"
...
echo "$0: rescoring lattices with AmMfa model in $srcdir/$iter.mdl"
$cmd JOB=1:$nj $dir/log/rescore.JOB.log \
  am-mfa2-rescore-lattice $spkvecs_opt $srcdir/$iter.mdl "ark:gunzip -c $olddir/lat.JOB.gz|" "$feats" \
