...
if [ $# != 4 ]; then
   echo "usage: steps/align_ammfa.sh <data-dir> <lang-dir> <src-dir> <align-dir>"
   echo "e.g.:  steps/align_ammfa.sh --transform-dir exp/tri3b data/train data/lang \\"
...
if [ $alimdl == $mdl ]; then 
  ...
  if [ $stage -le 2 ]; then
    ...
    $cmd JOB=1:$nj $dir/log/align.JOB.log  \
       am-mfa-align-compiled $scale_opts --beam=$beam --retry-beam=$retry_beam \
...
if [ $stage -le 2 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/align_pass1.JOB.log \
    am-mfa-align-compiled $scale_opts --beam=$beam --retry-beam=$retry_beam $alimdl \
...
if [ $stage -le 3 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/spk_vecs1.JOB.log \
    ...
    am-mfa-post-to-gpost $alimdl "$feats" ark:- ark:- \| \
...
if [ $stage -le 4 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/spk_vecs2.JOB.log \
    ali-to-post "ark:gunzip -c $dir/pre_ali.JOB.gz|" ark:- \| \
    weight-silence-post 0.0 $silphonelist $alimdl ark:- ark:- \| \
    am-mfa-est-spkvecs --spk2utt=ark:$sdata/JOB/spk2utt \
