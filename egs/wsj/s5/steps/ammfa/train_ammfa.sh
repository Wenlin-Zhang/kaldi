...
num_iters=20   # Total number of iterations
num_iters_alimdl=3 # Number of iterations for estimating alignment model.
realign_iters="3 6 9 15" # Iters to realign on.
   ...
   if [ $stage -le $x ]; then
     ...
     if [ $x -ge 1 ]; then
       ...
       while [ $inner_cnt -lt $inner_iters ]; do
         ...
         am-mfa-sum-accs $dir/$x.tw.$inner_cnt.acc $dir/$x.*.tw.$inner_cnt.acc
         rm $dir/$x.*.tw.$inner_cnt.acc 2>/dev/null
         echo "$0: stage $x.1.$inner_cnt.2 update tw"
         $cmd $dir/log/update.$x.tw.$inner_cnt.log \
           am-mfa-est --update-flags="tw" --use-l1=$use_l1 $weight_opt  $dir/$[$x+1].tw.$inner_cnt.mdl $dir/$x.tw.$inner_cnt.acc \
     ...
     $cmd JOB=1:$nj $dir/log/acc.$x.JOB.y.log \
       am-mfa-acc-stats --ammfa-gselect-direct=$gselect_direct --utt2spk=ark:$sdata/JOB/utt2spk $spkvecs_opt --update-flags="y" $dir/$[$x+1].0.mdl "$feats" \
         "ark,s,cs:gunzip -c $dir/ali.JOB.gz | ali-to-post ark:- ark:-|" \
         $dir/$x.JOB.y.acc || exit 1;
     am-mfa-sum-accs $dir/$x.y.acc $dir/$x.*.y.acc
     rm $dir/$x.*.y.acc 2>/dev/null
     echo "$0: stage $x.3 update y"
     $cmd $dir/log/update.$x.y.log \
       am-mfa-est --update-flags="y" --use-l1=$use_l1 $weight_opt  $dir/$[$x+1].0.mdl $dir/$x.y.acc \
       $dir/$[$x+1].y.mdl || exit 1;

     echo "$0: stage $x.4 accumulate statistics for update M"
     $cmd JOB=1:$nj $dir/log/acc.$x.JOB.M.log \
       am-mfa-acc-stats --ammfa-gselect-direct=$gselect_direct --utt2spk=ark:$sdata/JOB/utt2spk $spkvecs_opt --update-flags="M" $dir/$[$x+1].y.mdl "$feats" \
         "ark,s,cs:gunzip -c $dir/ali.JOB.gz | ali-to-post ark:- ark:-|" \
         $dir/$x.JOB.M.acc || exit 1;
     am-mfa-sum-accs $dir/$x.M.acc $dir/$x.*.M.acc
     rm $dir/$x.*.M.acc 2>/dev/null
     echo "$0: stage $x.5 update M"
     $cmd $dir/log/update.$x.M.log \
       am-mfa-est --update-flags="M" --use-l1=$use_l1 $weight_opt  $dir/$[$x+1].y.mdl $dir/$x.M.acc \
       $dir/$[$x+1].M.mdl || exit 1;

     echo "$0: stage $x.6 accumulate statistics for update mS"
     $cmd JOB=1:$nj $dir/log/acc.$x.JOB.mS.log \
       am-mfa-acc-stats --ammfa-gselect-direct=$gselect_direct --utt2spk=ark:$sdata/JOB/utt2spk $spkvecs_opt --update-flags="mS" $dir/$[$x+1].M.mdl "$feats" \
         "ark,s,cs:gunzip -c $dir/ali.JOB.gz | ali-to-post ark:- ark:-|" \
         $dir/$x.JOB.mS.acc || exit 1;
     am-mfa-sum-accs $dir/$x.mS.acc $dir/$x.*.mS.acc
     rm $dir/$x.*.mS.acc 2>/dev/null
     echo "$0: stage $x.7 update mS"
     $cmd $dir/log/update.$x.mS.log \
       am-mfa-est --update-flags="mS" --use-l1=$use_l1 $weight_opt  $dir/$[$x+1].M.mdl $dir/$x.mS.acc \
       $dir/$[$x+1].mS.mdl || exit 1;
     
     if [ $spk_dim -gt 0 -a $x -ge $min_iter_for_spk_update ]; then # update the speaker subspace
       echo "$0: stage $x.9 accumulate for the speaker subspace ..."
       if [ $stage -le $x ]; then
         echo "$0: accumulate statistics for speaker subspace ..."
         $cmd JOB=1:$nj $dir/log/acc.$x.JOB.N.log \
           am-mfa-acc-stats --ammfa-gselect-direct=$gselect_direct --utt2spk=ark:$sdata/JOB/utt2spk $spkvecs_opt --update-flags="N" $dir/$[$x+1].mS.mdl "$feats" \
            "ark,s,cs:gunzip -c $dir/ali.JOB.gz | ali-to-post ark:- ark:-|" \
            $dir/$x.JOB.N.acc || exit 1;
         am-mfa-sum-accs $dir/$x.N.acc $dir/$x.*.N.acc
         rm $dir/$x.*.N.acc 2>/dev/null
         echo "$0: stage $x.10 update the speaker subspace ..."
         $cmd $dir/log/update.$x.N.log \
           am-mfa-est --update-flags="N" --use-l1=$use_l1 $weight_opt  $dir/$[$x+1].mS.mdl $dir/$x.N.acc \
           $dir/$[$x+1].N.mdl || exit 1;
         mv $dir/$[$x+1].N.mdl $dir/$[$x+1].mdl
         rm $dir/$[$x+1].*.mdl
       fi
     else
       mv $dir/$[$x+1].mS.mdl $dir/$[$x+1].mdl
       rm $dir/$[$x+1].*.mdl
     fi
   fi
  ...
  while [ $x -lt $[$num_iters+$num_iters_alimdl] ]; do
    ...
    if [ $stage -le $x ]; then
      if [ $x -eq $num_iters ]; then
        ...
        $cmd JOB=1:$nj $dir/log/acc.$x.JOB.MmS.log \
          am-mfa-acc-stats-gpost --update-flags=MmS $cur_alimdl "$feats" \
            "ark,s,cs:gunzip -c $dir/ali.gpost.JOB.gz |" \
            $dir/$x.JOB.MmS.aliacc || exit 1;
        am-mfa-sum-accs $dir/$x.MmS.aliacc $dir/$x.*.MmS.aliacc
        rm $dir/$x.*.MmS.aliacc 2>/dev/null
        $cmd $dir/log/update_ali.$x.MmS.log \
          am-mfa-est --update-flags=MmS --remove-speaker-space=true --use-l1=$use_l1 $weight_opt $cur_alimdl \
          $dir/$x.M.aliacc $dir/$[$x+1].alimdl || exit 1;
      else
        echo "(pass $x) accumulate and update the phone vectors, phone projections, mean and covariance..."
        $cmd JOB=1:$nj $dir/log/acc.$x.JOB.yMmS.log \
        am-mfa-acc-stats-gpost --update-flags=yMmS $cur_alimdl "$feats" \
          "ark,s,cs:gunzip -c $dir/ali.gpost.JOB.gz |" \
          $dir/$x.JOB.yMmS.aliacc || exit 1;
        am-mfa-sum-accs $dir/$x.yMmS.aliacc $dir/$x.*.yMmS.aliacc
        rm $dir/$x.*.yMmS.aliacc 2>/dev/null
        $cmd $dir/log/update_ali.$x.yMmS.log \
          am-mfa-est --update-flags=yMmS --remove-speaker-space=true --use-l1=$use_l1 $weight_opt $cur_alimdl \
          $dir/$x.yMmS.aliacc $dir/$[$x+1].alimdl || exit 1;
      fi
