...
# This script does decoding with an AmMfa system, by rescoring lattices
# generated from a previous AmMfa system.  Basically it rescores
...
if [ $# -ne 4 ]; then
  echo "Usage: steps/decode_ammfa_rescore.sh [options] <graph-dir|lang-dir> <data-dir> <old-decode-dir> <decode-dir>"
  echo " e.g.: steps/decode_ammfa_rescore.sh --transform-dir exp/tri3b/decode_dev93_tgpr \\"
...
$cmd JOB=1:$nj $dir/log/rescore.JOB.log \
  am-mfa-rescore-lattice $spkvecs_opt $srcdir/$iter.mdl "ark:gunzip -c $olddir/lat.JOB.gz|" "$feats" \
